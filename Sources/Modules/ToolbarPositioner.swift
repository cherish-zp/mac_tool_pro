...
/// 工具条定位器：计算工具条在选区下方的屏幕坐标位置。
/// 纯函数，无副作用，便于单测。
public enum ToolbarPositioner {
    /// 计算工具条原点（屏幕坐标），固定在选区下方。
    /// 下方放不下时回退到选区上方。
    /// - Parameters:
    ...
    public static func position(
        ...
        // 固定放选区下方
        var originY = rect.minY - toolbarSize.height - margin + screenFrame.origin.y

        // 下方放不下则回退到选区上方
        if originY < screenFrame.minY {
            originY = rect.maxY + margin + screenFrame.origin.y
        }
